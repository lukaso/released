...
describe('parseInput — popular-projects alias shorthand removed', () => {
  // The curated alias catalog (KNOWN_PROJECTS) and the alias-resolution
  // parser were removed: `react`/`gtk`/`next.js` are no longer repo
  // shortcuts. These guards prevent accidental reintroduction — a two-token
  // input with no owner/repo, or a bare alias alone, now falls through to
  // InvalidInputError instead of resolving to a known repo.

  it('does NOT resolve "react abc1234" (alias removed)', () => {
    expect(() => parseInput('react abc1234')).toThrow(InvalidInputError);
  });

  it('does NOT resolve "gtk 8c0ef808ea" (GitLab alias removed)', () => {
    expect(() => parseInput('gtk 8c0ef808ea')).toThrow(InvalidInputError);
  });

  it('does NOT resolve "gtk@8c0ef808ea" (no-space alias form removed)', () => {
    expect(() => parseInput('gtk@8c0ef808ea')).toThrow(InvalidInputError);
  });

  it('does NOT resolve the two-arg alias ("gtk", "8c0ef808ea")', () => {
    expect(() => parseInput('gtk', '8c0ef808ea')).toThrow(InvalidInputError);
  });

  it('unknown single word throws InvalidInputError (no alias lookup)', () => {
    expect(() => parseInput('totallyfake')).toThrow(InvalidInputError);
  });

  it('bare alias alone throws InvalidInputError (no SHA/PR context)', () => {
    expect(() => parseInput('gtk')).toThrow(InvalidInputError);
  });

  it('two non-owner/repo tokens throw InvalidInputError', () => {
    expect(() => parseInput('gtk hello')).toThrow(InvalidInputError);
  });

  it('bare SHA alone still throws BareShaError (unchanged)', () => {
