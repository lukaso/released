...
if [ "$ACTION" = push ]; then
  ...
  # (1) git semantics: the FIRST positional is the REMOTE (git's <repository>) — a remote merely
  ...
  for tok in $positionals; do
    if [ -n "$first" ]; then
      first=""
      case "$tok" in *:*) explicit=1; [ -n "${tok##*:}" ] && is_default_name "${tok##*:}" && gated=1;; esac
      continue
    fi
    explicit=1
    case "$tok" in
  ...
  if [ "$gated" = 0 ]; then
    if [ -n "$explicit" ]; then
      : # explicit refspec destination(s), none default -> feature target -> allow
    else
