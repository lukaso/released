...
deny() {
  # control chars (a newline-y file path, say) would break the JSON — flatten them first (#67 notes)
...
# Quick reject: no git commit/push anywhere -> not our concern (the segment walk below decides).
printf '%s' "$low" | grep -Eq '(^|[;&|([:space:]])git[[:space:]]+([^;&|]*[[:space:]])?(commit|push)([[:space:]]|$)' || exit 0

def_lc=$(printf '%s' "$DEF" | tr '[:upper:]' '[:lower:]')
is_default_name() { [ "$1" = "$def_lc" ] || [ "$1" = master ]; }

# Effective repo dir for a bare push / commit: `git -C <dir>` (uppercase C — the DIR flag;
# lowercase -c is a k=v CONFIG pair and must be ignored, or it poisons branch resolution:
# the released#65 review finding), else a leading `cd <dir>`, else cwd. Both accept a
# double-quoted path (spaces) — an unquoted-only parse fail-closed on real checkouts (#67 notes).
effective_dir() {
  local d="$HCWD"
  if printf '%s' "$CMD" | grep -Eq '(^|[[:space:]])git[[:space:]]+-C[[:space:]]+"'; then
    d=$(printf '%s' "$CMD" | sed -E 's@(^|.*[[:space:]])git[[:space:]]+-C[[:space:]]+"([^"]+)".*@\2@')
  elif printf '%s' "$CMD" | grep -Eq '(^|[[:space:]])git[[:space:]]+-C[[:space:]]'; then
    d=$(printf '%s' "$CMD" | sed -E 's@(^|.*[[:space:]])git[[:space:]]+-C[[:space:]]+([^[:space:]]+).*@\2@')
  elif printf '%s' "$CMD" | grep -Eq '^[[:space:]]*cd[[:space:]]+"'; then
    d=$(printf '%s' "$CMD" | sed -E 's@^[[:space:]]*cd[[:space:]]+"([^"]+)".*@\1@')
  elif printf '%s' "$CMD" | grep -Eq '^[[:space:]]*cd[[:space:]]'; then
...
# ---- decide whether any TARGET is the default branch (refspec-aware, PER SEGMENT) ----
# gated=1 means "this acts on the default branch -> needs the approval phrase".
# Segment-aware because a compound command hides earlier pushes from a whole-line parse:
# `git push origin main && git push origin feat` must gate on the FIRST segment (found by the
# bot's own review of released#66 — engine-feedback, confirmed by execution).

# current_is_default -> 0 default / 1 feature / 2 unresolvable
current_is_default() {
  local cur; cur="$(resolve_current)"
  [ -z "$cur" ] && return 2
  is_default_name "$(printf '%s' "$cur" | tr '[:upper:]' '[:lower:]')" && return 0
  return 1
}
# dst_check CANDIDATE — normalize one push destination, set gated=1 if it is the default.
# Normalization (all confirmed bypasses): strip ONE leading `+` (the force marker), strip a
# `refs/heads/` prefix, and treat HEAD/@ as "the CURRENT branch" (resolve it; fail closed).
dst_check() {
  local t="${1#+}"; t="${t#refs/heads/}"
  [ -n "$t" ] || return 0
  case "$t" in
    head|@) current_is_default; case $? in 0|2) gated=1;; esac ;;   # unresolvable HEAD -> fail closed
    *) is_default_name "$t" && gated=1 ;;
  esac
  return 0
}
# seg_parse SEG — classify a segment by git's ACTUAL subcommand: the first positional after
# `git` and its global flags. Grepping for `push`/`commit` ANYWHERE mis-read words inside a
# `-m` message as arguments (the #67 finding: a commit message saying "push" bypassed the
# commit gate on main AND wrongly denied a feature commit quoting a push command).
# Sets SEG_SUB (the subcommand, or "") and SEG_ARGS (the tokens after it).
seg_parse() {
  SEG_SUB="" SEG_ARGS=""
  local tok seen_git="" skip=""
  for tok in $1; do
    if [ -n "$SEG_SUB" ]; then SEG_ARGS="$SEG_ARGS $tok"; continue; fi
    if [ -z "$seen_git" ]; then
      case "$tok" in git|*/git) seen_git=1;; esac
      continue
    fi
    if [ -n "$skip" ]; then skip=""; continue; fi
    case "$tok" in
      -C|-c|--git-dir|--work-tree|--namespace|--exec-path) skip=1;;   # global flag w/ separate value
      -*) :;;                                                         # other global flag
      *) SEG_SUB="$tok";;
    esac
  done
}
# check_push_segment — the refspec-aware push-target decision for the CURRENT SEG_ARGS.
check_push_segment() {
  local positionals tok first explicit
  positionals=""
  for tok in $SEG_ARGS; do
    case "$tok" in -*) continue;; esac
    positionals="$positionals $tok"
  done
  # git semantics: the FIRST positional is the REMOTE (git's <repository>) — a remote merely
  # NAMED like the default must not gate (released#65 review). Only the refspecs AFTER it name a
  # destination. A `:` form in the remote slot is still dst-checked (belt: never let `X:main`
  # by) but does NOT count as an explicit refspec — ssh/https remotes contain colons, and
  # treating them as refspecs suppressed the current-branch fallback (the #67 URL bypass:
  # `git push git@host:o/r.git` on main sailed through).
  first=1 explicit=""
  for tok in $positionals; do
    if [ -n "$first" ]; then
      first=""
      case "$tok" in *:*) dst_check "${tok##*:}";; esac
      continue
    fi
    explicit=1
    case "$tok" in
  ...
  if [ "$gated" = 0 ] && [ -z "$explicit" ]; then
    # bare push, or push to a named/URL remote -> destination is the current branch
    current_is_default
    case $? in
      0) gated=1;;
      2) gated=1;;   # fail-closed: a push we cannot prove is a feature branch needs the phrase
    esac
  fi
}

gated=0 ACTION="" commit_seen=""
while IFS= read -r seg; do
  [ -n "$seg" ] || continue
  seg_parse "$seg"
  case "$SEG_SUB" in
    push)   check_push_segment
            [ "$gated" = 1 ] && { ACTION=push; break; } ;;
    commit) commit_seen=1 ;;
  esac
done <<SEGS
$(printf '%s\n' "$low" | tr ';&|()' '\n\n\n\n\n')
SEGS

if [ "$gated" = 0 ] && [ -n "$commit_seen" ]; then
